...
type StorageClass struct {
	...
	FreeSizeGB               int    `json:"free_size_gb"`
...
// FloatingIP represents a floating IP from the API.
type FloatingIP struct {
	ID          int     `json:"id"`
	IPUUID      string  `json:"ip_uuid"`
...
type FloatingIPReserveResponse struct {
	ID          int     `json:"id"`
	IPUUID      string  `json:"ip_uuid"`
	IPAddress   string  `json:"ip_address"`
	IPv6Address *string `json:"ipv6_address"`
...
// FloatingIPAttachResponse represents POST /floating-ips/{uuid}/attach response.
type FloatingIPAttachResponse struct {
	IPUUID    string `json:"ip_uuid"`
	IPAddress string `json:"ip_address"`
	VMUUID    string `json:"vm_uuid"`
	Status    string `json:"status"`
...
// VMNic represents one network interface of a VM (GET /vms/{uuid}/nics).
type VMNic struct {
...
// VMNicListResponse represents GET /vms/{uuid}/nics response.
type VMNicListResponse struct {
	Items []VMNic `json:"items"`
	Total int     `json:"total"`
}

// VMNetworkAttachResponse represents POST /vms/{uuid}/network/attach response.
type VMNetworkAttachResponse struct {
	Status      string `json:"status"`
	VMUUID      string `json:"vm_uuid"`
	NetworkUUID string `json:"network_uuid"`
	InterfaceID string `json:"interface_id"`
